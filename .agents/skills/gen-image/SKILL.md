---
name: gen-image-cli
description: Generate images inside Codex (CLI) by calling the locally installed Codex CLI via Bash (backed by OpenAI Image-2, API model gpt-image-2), saving outputs to ./images/ in the current working directory. Only for use inside the Codex environment. Trigger words: generate image, draw an image, make an image, 生圖.
allowed-tools: Bash(codex:*) Bash(mkdir:*) Bash(ls:*) Bash(pwd:*)
---

# Image Generation Skill (Codex CLI version)

**For use only in the Codex terminal environment** — this Skill calls the user's locally installed Codex CLI directly via Bash. If you are in the Codex desktop app / Cowork window, use the `gen-image-app` Skill instead.
